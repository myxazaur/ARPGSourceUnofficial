...
public class AmbientOcclusionFace {
   ...
   public void updateVertexBrightness(IBlockAccess worldIn, IBlockState state, BlockPos centerPos, EnumFacing direction, float[] faceShape, BitSet shapeState) {
      ...
      PooledMutableBlockPos blockpos$pooledmutableblockpos1 = PooledMutableBlockPos.retain(blockpos)
         .move(blockmodelrenderer$enumneighborinfo.corners[0]);
      PooledMutableBlockPos blockpos$pooledmutableblockpos2 = PooledMutableBlockPos.retain(blockpos)
         .move(blockmodelrenderer$enumneighborinfo.corners[1]);
      PooledMutableBlockPos blockpos$pooledmutableblockpos3 = PooledMutableBlockPos.retain(blockpos)
         .move(blockmodelrenderer$enumneighborinfo.corners[2]);
      PooledMutableBlockPos blockpos$pooledmutableblockpos4 = PooledMutableBlockPos.retain(blockpos)
         .move(blockmodelrenderer$enumneighborinfo.corners[3]);
      int i = lightmapCoord(worldIn, state, blockpos$pooledmutableblockpos1);
      int j = lightmapCoord(worldIn, state, blockpos$pooledmutableblockpos2);
      int k = lightmapCoord(worldIn, state, blockpos$pooledmutableblockpos3);
      int l = lightmapCoord(worldIn, state, blockpos$pooledmutableblockpos4);
      float f = getAOLight(worldIn, blockpos$pooledmutableblockpos1);
      float f1 = getAOLight(worldIn, blockpos$pooledmutableblockpos2);
      float f2 = getAOLight(worldIn, blockpos$pooledmutableblockpos3);
      float f3 = getAOLight(worldIn, blockpos$pooledmutableblockpos4);
      ...
      if (!flag2 && !flag) {
         ...
         BlockPos blockpos1 = blockpos$pooledmutableblockpos.setPos(blockpos$pooledmutableblockpos1)
            .move(blockmodelrenderer$enumneighborinfo.corners[2]);
         f4 = getAOLight(worldIn, blockpos1);
         i1 = lightmapCoord(worldIn, state, blockpos1);
      }
      ...
      if (!flag3 && !flag) {
         ...
         BlockPos blockpos2 = blockpos$pooledmutableblockpos.setPos(blockpos$pooledmutableblockpos1)
            .move(blockmodelrenderer$enumneighborinfo.corners[3]);
         f5 = getAOLight(worldIn, blockpos2);
         j1 = lightmapCoord(worldIn, state, blockpos2);
      }
      ...
      if (!flag2 && !flag1) {
         ...
         BlockPos blockpos3 = blockpos$pooledmutableblockpos.setPos(blockpos$pooledmutableblockpos2)
            .move(blockmodelrenderer$enumneighborinfo.corners[2]);
         f6 = getAOLight(worldIn, blockpos3);
         k1 = lightmapCoord(worldIn, state, blockpos3);
      }
      ...
      if (!flag3 && !flag1) {
         ...
         BlockPos blockpos4 = blockpos$pooledmutableblockpos.setPos(blockpos$pooledmutableblockpos2)
            .move(blockmodelrenderer$enumneighborinfo.corners[3]);
         f7 = getAOLight(worldIn, blockpos4);
         l1 = lightmapCoord(worldIn, state, blockpos4);
      }

      int i3 = lightmapCoord(worldIn, state, centerPos);
      if (shapeState.get(0) || !worldIn.getBlockState(centerPos.offset(direction)).isOpaqueCube()) {
         i3 = lightmapCoord(worldIn, state, centerPos.offset(direction));
      }

      float f8 = shapeState.get(0) ? getAOLight(worldIn, blockpos) : getAOLight(worldIn, centerPos);
   ...
   public static final float getAOLight(IBlockAccess world, BlockPos pos) {
      IBlockState state = world.getBlockState(pos);
      return state.getAmbientOcclusionLightValue();
   }

   public static int lightmapCoord(IBlockAccess world, IBlockState state1, BlockPos pos) {
      IBlockState state = world.getBlockState(pos);
      int light = state.getLightValue(world, pos);
      if (light > 0) {
         int x = ColorConverters.UnpackLightmapCoordsX(state.getPackedLightmapCoords(world, pos));
         return ColorConverters.RGBtoDecimal255(Math.max(120 + light * 8, x), 0, 120 + light * 8);
      } else {
         return state.getPackedLightmapCoords(world, pos);
      }
   }

   public int getAoBrightness(int br1, int br2, int br3, int br4) {
      if (br1 == 0) {
         br1 = br4;
      }
   ...
   public int getVertexBrightness(
      ...
      int i = (int)(
         ...
         & 0xFF;
      int j = (int)(
            (p_178203_1_ & 0xFF) * p_178203_5_ + (p_178203_2_ & 0xFF) * p_178203_6_ + (p_178203_3_ & 0xFF) * p_178203_7_ + (p_178203_4_ & 0xFF) * p_178203_8_
         )
         & 0xFF;
      return i << 16 | j;
   }
   ...
   public static enum EnumNeighborInfo {
      ...
      public final EnumFacing[] corners;
      public final float shadeWeight;
      public final boolean doNonCubicWeight;
      public final Orientation[] vert0Weights;
      public final Orientation[] vert1Weights;
      public final Orientation[] vert2Weights;
      public final Orientation[] vert3Weights;
      public static final EnumNeighborInfo[] VALUES = new EnumNeighborInfo[6];

      private EnumNeighborInfo(
         EnumFacing[] p_i46236_3_,
         float p_i46236_4_,
         boolean p_i46236_5_,
         Orientation[] p_i46236_6_,
         Orientation[] p_i46236_7_,
         Orientation[] p_i46236_8_,
         Orientation[] p_i46236_9_
      ) {
   ...
   static enum VertexTranslations {
      ...
      public static final VertexTranslations[] VALUES = new VertexTranslations[6];
