...
public class RenderNetherSmoke extends WeatherRenderList {
   static ResourceLocation texture = new ResourceLocation("arpg:textures/largesmoke.png");

   public RenderNetherSmoke(WeatherPhenomenon weather) {
      super(weather);
   }
   ...
   public void render(double x, double y, double z, float rotationY, float rotationX, float partialTicks) {
      if (!this.isListsEmpty()) {
         for (int i = 0; i < this.listX.size(); i++) {
            this.doRender(this.listX.get(i) - x, 60.0 - y, this.listZ.get(i) - z, partialTicks, this.listY.get(i), rotationY, rotationX);
         }
      }
   }
   ...
   public void doRender(double x, double y, double z, float partialTicks, double noisevalue, float rotationY, float rotationX) {
      ...
      GlStateManager.rotate((float)(Math.sin(noisevalue) * 480.0), 0.0F, 0.0F, 1.0F);
      Minecraft.getMinecraft().getTextureManager().bindTexture(texture);
      float scale = (float)(3.5 + noisevalue * 35.0);
