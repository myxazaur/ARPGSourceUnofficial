...
public class BiomeProviderAquatica extends BiomeProvider {
   private GenLayer genBiomes;
   private GenLayer biomeIndexLayer;
   private BiomeCache biomeCache = new BiomeCache(this);
   private ChunkGeneratorSettings settings2;

   public BiomeProviderAquatica() {
   }

   public BiomeProviderAquatica(long seed, WorldType worldTypeIn, String options) {
      this();
      if (worldTypeIn == WorldType.CUSTOMIZED && !options.isEmpty()) {
         this.settings2 = Factory.jsonToFactory(options).build();
      }

      GenLayer[] agenlayer = GenLayerFixAquatica.initializeAllBiomeGenerators(seed, worldTypeIn, this.settings2);
      this.genBiomes = agenlayer[0];
      this.biomeIndexLayer = agenlayer[1];
   }

   public BiomeProviderAquatica(WorldInfo info) {
