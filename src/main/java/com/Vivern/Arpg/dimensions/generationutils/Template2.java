...
public class Template2 {
   ...
   public Template2(Template template) {
   ...
   public void takeBlocksFromWorld(World worldIn, BlockPos startPos, BlockPos endPos, boolean takeEntities, @Nullable Block toIgnore) {
      if (endPos.getX() >= 1 && endPos.getY() >= 1 && endPos.getZ() >= 1) {
         ...
         for (MutableBlockPos blockpos$mutableblockpos : BlockPos.getAllInBoxMutable(blockpos1, blockpos2)) {
   ...
   private void takeEntitiesFromWorld(World worldIn, BlockPos startPos, BlockPos endPos) {
      ...
      for (Entity entity : list) {
         Vec3d vec3d = new Vec3d(
            entity.posX - startPos.getX(), entity.posY - startPos.getY(), entity.posZ - startPos.getZ()
         );
      ...
      for (BlockInfo template$blockinfo : this.blocks) {
         ...
         if (structureboundingbox == null || structureboundingbox.isVecInside(blockpos)) {
            ...
            if (iblockstate.getBlock() == Blocks.STRUCTURE_BLOCK && template$blockinfo.tileentityData != null) {
               Mode tileentitystructure$mode = Mode.valueOf(template$blockinfo.tileentityData.getString("mode"));
   ...
   public void addBlocksToWorldGenAdvanced(WorldGenAdvanced worldIn, BlockPos pos, PlacementSettings placementIn, int flags) {
      if ((!this.blocks.isEmpty() || !placementIn.getIgnoreEntities() && !this.entities.isEmpty())
         ...
         for (BlockInfo template$blockinfo : this.blocks) {
            BlockPos blockpos = transformedBlockPos(placementIn, template$blockinfo.pos).add(pos);
            if ((structureboundingbox == null || structureboundingbox.isVecInside(blockpos)) && template$blockinfo != null) {
   ...
   public NBTTagCompound writeToNBT(NBTTagCompound nbt) {
      ...
      for (BlockInfo template$blockinfo : this.blocks) {
         ...
         nbttagcompound.setTag(
            "pos", this.writeInts(template$blockinfo.pos.getX(), template$blockinfo.pos.getY(), template$blockinfo.pos.getZ())
         );
      ...
      for (EntityInfo template$entityinfo : this.entities) {
         ...
         nbttagcompound1.setTag(
            "pos", this.writeDoubles(template$entityinfo.pos.x, template$entityinfo.pos.y, template$entityinfo.pos.z)
         );
         nbttagcompound1.setTag(
            "blockPos",
            this.writeInts(
               template$entityinfo.blockPos.getX(), template$entityinfo.blockPos.getY(), template$entityinfo.blockPos.getZ()
            )
         );
