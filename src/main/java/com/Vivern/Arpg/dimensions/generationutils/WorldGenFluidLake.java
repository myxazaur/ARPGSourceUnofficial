...
public class WorldGenFluidLake extends WorldGenerator {
   public Block liquid;
   public Block stone;
   public Block ice;
   public Block dirt;
   public boolean freeze;
   public boolean lavaStyle;
   public int size = 4;

   public WorldGenFluidLake(Block liquid, Block stone, Block ice, Block dirt, boolean freeze, boolean stoneAround) {
      this.liquid = liquid;
      this.stone = stone;
      this.ice = ice;
      this.dirt = dirt;
            ...
            for (int l2 = 0; l2 < 16; l2++) {
               for (int k = 0; k < 8; k++) {
                  ...
                  if (flag) {
                     ...
                     if (k < 4 && !material.isSolid() && worldIn.getBlockState(position.add(k1, k, l2)).getBlock() != this.liquid) {
                        return false;
                     }
                  }
               }
            ...
            for (int i3 = 0; i3 < 16; i3++) {
               for (int i4 = 0; i4 < 8; i4++) {
                  if (aboolean[(l1 * 16 + i3) * 8 + i4]) {
                     worldIn.setBlockState(position.add(l1, i4, i3), i4 >= 4 ? Blocks.AIR.getDefaultState() : this.liquid.getDefaultState(), 2);
                  }
               }
            }
         }
            ...
            for (int j3 = 0; j3 < 16; j3++) {
               for (int j4 = 4; j4 < 8; j4++) {
                  if (aboolean[(i2 * 16 + j3) * 8 + j4]) {
                     ...
                     if (worldIn.getBlockState(blockpos).getBlock() == this.dirt
                        && worldIn.getLightFor(EnumSkyBlock.SKY, position.add(i2, j4, j3)) > 0) {
                        Biome biome = worldIn.getBiome(blockpos);
                        worldIn.setBlockState(blockpos, biome.topBlock, 2);
                     }
                  }
               }
            }
               ...
               for (int k3 = 0; k3 < 16; k3++) {
                  for (int k4 = 0; k4 < 8; k4++) {
                     ...
                     if (flag1 && (k4 < 4 || rand.nextInt(2) != 0) && worldIn.getBlockState(position.add(j2, k4, k3)).getMaterial().isSolid()) {
                        worldIn.setBlockState(position.add(j2, k4, k3), this.stone.getDefaultState(), 2);
                     }
                  }
               }
            }
            ...
            for (int k2 = 0; k2 < 16; k2++) {
               for (int l3 = 0; l3 < 16; l3++) {
                  ...
                  if (worldIn.canBlockFreezeWater(position.add(k2, 4, l3))) {
                     ...
                     worldIn.setBlockState(position.add(k2, 4, l3), this.ice.getDefaultState(), flag);
                  }
               }
            }
         }
