...
public class DungeonCleftsMapGen extends MapGenBase {
   ...
   protected void addTunnel(
      ...
      float save14 = p_180702_14_;
      int save15 = p_180702_15_;
      int save16 = p_180702_16_;
      ...
      for (boolean flag = random.nextInt(8) == 0; p_180702_15_ < p_180702_16_; p_180702_15_++) {
         double d2 = 1.7 + MathHelper.sin(p_180702_15_ * (float) Math.PI / p_180702_16_) * p_180702_12_;
         ...
         f += (random.nextFloat() - random.nextFloat()) * random.nextFloat() * 3.0F;
            ...
            if (p_180702_6_ >= d0 - 16.0 - d2 * 2.0
               ...
               if (!flag3) {
                  ...
                  for (int j3 = k2; j3 < k; j3++) {
                     double d10 = (j3 + p_180702_3_ * 16 + 0.5 - p_180702_6_) / d2;

                     for (int i2 = i3; i2 < i1; i2++) {
                        double d8 = (i2 + p_180702_4_ * 16 + 0.5 - p_180702_10_) / d2;
                        boolean flag1 = false;
   ...
   protected void recursiveGenerate(World worldIn, int chunkX, int chunkZ, int originalX, int originalZ, ChunkPrimer chunkPrimerIn) {
      ...
      for (int j = 0; j < i; j++) {
         ...
         double d1 = this.rand.nextInt(this.rand.nextInt(256) + 8);
         double d2 = chunkZ * 16 + this.rand.nextInt(16);
         int k = 1;
         if (this.rand.nextInt(3) == 0) {
            this.addRoom(this.rand.nextLong(), originalX, originalZ, chunkPrimerIn, d0, d1, d2);
            k += this.rand.nextInt(7);
         }

         for (int l = 0; l < k; l++) {
            float f = this.rand.nextFloat() * (float) (Math.PI * 2);
            float f1 = (float)this.rand.nextGaussian();
            float f2 = this.rand.nextFloat() * 4.0F + this.rand.nextFloat();
            if (this.rand.nextInt(10) == 0) {
               f2 *= this.rand.nextFloat() * this.rand.nextFloat() * 2.0F + 1.0F;
            }

            this.addTunnel(this.rand.nextLong(), originalX, originalZ, chunkPrimerIn, d0, d1, d2, f2, f, f1, 0, 100, 4.0);
         }
      }
   }
   ...
   private boolean isExceptionBiome(Biome biome) {
      return biome == Biomes.BEACH ? true : biome == Biomes.DESERT;
   }
