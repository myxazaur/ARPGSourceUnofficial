...
public class BiomeProviderFrost extends BiomeProvider {
   private GenLayer genBiomes;
   private GenLayer biomeIndexLayer;
   private BiomeCache biomeCache = new BiomeCache(this);

   public BiomeProviderFrost() {
   }

   public BiomeProviderFrost(long seed) {
      this();
      GenLayer[] agenlayer = GenLayerFixEthernalfrost.createWorld(seed);
      this.genBiomes = agenlayer[0];
      this.biomeIndexLayer = agenlayer[1];
   }

   public BiomeProviderFrost(World world) {
      this(world.getSeed());
   }
   ...
   public Biome[] getBiomesForGeneration(Biome[] par1ArrayOfBiome, int par2, int par3, int par4, int par5) {
      ...
      try {
         for (int i = 0; i < par4 * par5; i++) {
            par1ArrayOfBiome[i] = Biome.getBiome(aint[i], BiomesRegister.FROZEN_MOUNTAINS[0]);
         }
   ...
   public int[] getBiomesForGenerationRaw(int[] par1ArrayOfBiome, int par2, int par3, int par4, int par5) {
      IntCache.resetIntCache();
      if (par1ArrayOfBiome == null || par1ArrayOfBiome.length < par4 * par5) {
         par1ArrayOfBiome = new int[par4 * par5];
      }
   ...
   public Biome[] getBiomes(Biome[] listToReuse, int x, int y, int width, int length, boolean cacheFlag) {
      ...
      if (cacheFlag && width == 16 && length == 16 && (x & 15) == 0 && (y & 15) == 0) {
         ...
         for (int i = 0; i < width * length; i++) {
            listToReuse[i] = Biome.getBiome(aint[i], BiomesRegister.FROZEN_MOUNTAINS[0]);
         }
   ...
   public int[] getBiomesRaw(int[] listToReuse, int x, int y, int width, int length) {
      IntCache.resetIntCache();
      if (listToReuse == null || listToReuse.length < width * length) {
   ...
   public boolean areBiomesViable(int x, int z, int radius, List<Biome> allowed) {
      ...
      try {
         for (int k1 = 0; k1 < i1 * j1; k1++) {
            Biome biome = Biome.getBiome(aint[k1], BiomesRegister.FROZEN_MOUNTAINS[0]);
   ...
   public BlockPos findBiomePosition(int x, int z, int range, List biomes, Random random) {
      ...
      for (int k2 = 0; k2 < l1 * i2; k2++) {
         int l2 = l + k2 % l1 << 2;
         int i3 = i1 + k2 / l1 << 2;
         Biome biome = Biome.getBiome(aint[k2], BiomesRegister.FROZEN_MOUNTAINS[0]);
