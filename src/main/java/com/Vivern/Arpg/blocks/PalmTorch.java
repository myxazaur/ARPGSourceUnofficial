...
public class PalmTorch extends Block {
   public static ResourceLocation res = new ResourceLocation("arpg:textures/palm_fire.png");
   ...
   public PalmTorch() {
      super(Material.WOOD);
      this.setRegistryName("palm_torch");
      this.setTranslationKey("palm_torch");
   ...
   public void randomDisplayTick(IBlockState stateIn, World worldIn, BlockPos pos, Random rand) {
      if (rand.nextFloat() < 0.9F) {
         ...
         double d1 = pos.getY() + 0.65;
         double d2 = pos.getZ() + 0.5;
         double d3 = 0.22;
         double d4 = 0.34;
         int livetime = 30 + rand.nextInt(30);
         float scale = 0.16F + rand.nextFloat() / 30.0F;
         float scaleTickAdding = -(scale / livetime);
         float randsp1 = (rand.nextFloat() - 0.5F) / 120.0F;
         float randsp3 = (rand.nextFloat() - 0.5F) / 120.0F;
         if (enumfacing.getAxis().isHorizontal()) {
            ...
            worldIn.spawnParticle(
               EnumParticleTypes.SMOKE_NORMAL, d0 + d4 * enumfacing1.getXOffset(), d1 + 0.22, d2 + d4 * enumfacing1.getZOffset(), 0.0, 0.0, 0.0, new int[0]
            );
         } else {
            GUNParticle spelll = new GUNParticle(
               res,
