...
public class AquaticaCavesMapGen extends MapGenBase {
   ...
   public boolean aquaMode;

   public AquaticaCavesMapGen() {
      this.aquaMode = false;
   }

   public AquaticaCavesMapGen(boolean aquaMode) {
      this.aquaMode = aquaMode;
   }
   ...
   protected void addTunnel(
      ...
      for (boolean flag = random.nextInt(6) == 0; p_180702_15_ < p_180702_16_; p_180702_15_++) {
         double d2 = (this.aquaMode ? 3.25 : 1.75) + MathHelper.sin(p_180702_15_ * (float) Math.PI / p_180702_16_) * p_180702_12_;
   ...
   protected boolean canReplaceBlock(IBlockState p_175793_1_, IBlockState p_175793_2_) {
      if (p_175793_1_.getBlock() != Blocks.WATER && p_175793_1_.getBlock() != Blocks.FLOWING_WATER) {
         return true;
      } else if (p_175793_1_.getBlock() == Blocks.STONE) {
         return true;
   ...
   protected void recursiveGenerate(World worldIn, int chunkX, int chunkZ, int originalX, int originalZ, ChunkPrimer chunkPrimerIn) {
      ...
      for (int j = 0; j < i; j++) {
         ...
         double d1 = this.rand.nextInt(this.rand.nextInt(this.aquaMode ? 115 : 230) + (this.aquaMode ? 70 : 8));
   ...
   protected boolean isOceanBlock(ChunkPrimer data, int x, int y, int z, int chunkX, int chunkZ) {
      ...
      return this.aquaMode ? block == Blocks.AIR : block == Blocks.FLOWING_WATER || block == Blocks.WATER;
   }
   ...
   protected void digBlock(ChunkPrimer data, int x, int y, int z, int chunkX, int chunkZ, boolean foundTop, IBlockState state, IBlockState up) {
      Biome biome = this.world.getBiome(new BlockPos(x + chunkX * 16, 0, z + chunkZ * 16));
      if (this.aquaMode || biome != BiomesRegister.SEAWEED_BAY) {
         IBlockState top = biome.topBlock;
         IBlockState filler = biome.fillerBlock;
         if (this.canReplaceBlock(state, up) || state.getBlock() == top.getBlock() || state.getBlock() == filler.getBlock()) {
            if (y < 7) {
               data.setBlockState(x, y, z, BLK_LAVA);
            } else if (this.aquaMode && y == 7 && up.getMaterial() == Material.WATER) {
               data.setBlockState(x, y, z, BLK_OBSIDIAN);
            } else if (this.aquaMode && y < 195 && y > 7) {
