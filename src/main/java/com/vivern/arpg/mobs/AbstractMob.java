...
public abstract class AbstractMob extends EntityMob implements IMultitexture {
   ...
   public static final void addToRegister(Class thismobClass, String name, int eggPrimary, int eggSecondary,
         int trackerRange, int updateFrequency) {
      ...
      if (this.canDropLoot && this.loot != null) {
         for (int i = 0; i < this.loot.length; i++) {
            ...
            if (this.rand.nextFloat() < drop.chance * this.getTamedDropChances(drop, i, lootingModifier, source)) {
               ...
               int count = lootadd + drop.mincount
                     + (drop.maxcount <= drop.mincount ? 0 : this.rand.nextInt(drop.maxcount - drop.mincount + 1));
               if (count > 0) {
                  ...
                  if (Math.abs(yy - this.posY) <= radius) {
                     ...
                     if (Math.abs(zz - this.posZ) <= radius) {
                        ...
                        if ("thermalexpansion:device".equals(state.getBlock().getRegistryName().toString())
                           ...
                           System.out.println(
                                 "Thermalexpansion creature encaptulator was trying to capture too powerful mob");
                           return;
                        }
                     }
                  }
            ...
            for (int l3 = j2; l3 < k2; l3++) {
               for (int i4 = l2; i4 < i3; i4++) {
                  for (int j4 = j3; j4 < k3; j4++) {
                     IBlockState iblockstate1 = this.world
                           .getBlockState(blockpos$pooledmutableblockpos.setPos(l3, i4, j4));
                     if (iblockstate1.getBlock() == Blocks.DISPENSER) {
                        ...
                        if (tilex != null && tilex instanceof TileEntityDispenser) {
                           ...
                           for (int i = 0; i < dispenser.getSizeInventory(); i++) {
                              if ("enderio:item_soul_vial"
                                    .equals(dispenser.getStackInSlot(i).getItem().getRegistryName().toString())) {
                                 hasVial = true;
                                 break;
                              }
                           }
                           ...
                           if (hasVial) {
                              ...
                              shouldReturn = true;
                              System.out.println(
                                    "Dispenser with enderio soul vials was trying to capture too powerful mob");
                           }
                        }
                     }
                  }
   ...
   public int getMobDifficulty() {
      return this.defaultteam != null && this.world != null && this.world.getDifficulty() != null
            && this.defaultteam.equals(this.team)
