...
public class Rainfall extends WorldEvent implements IVoxelCloudInfo {
   public static ResourceLocation tex = new ResourceLocation("arpg:textures/aquatica_rainfall.png");
   ...
   public Rainfall(AbstractWorldProvider worldProvider, int index, int livetimeMin, int livetimeMax, float chanceToStart) {
      super(worldProvider, index, livetimeMin, livetimeMax, chanceToStart);
      this.soundRain = Sounds.weather_rainfall;
      this.soundRainAbove = Sounds.weather_rainfall;
   }
   ...
   public void render(float partialTicks, WorldClient world, Minecraft mc) {
      ...
      WorldEventsHandler.renderRainSnow(partialTicks, this.rand, 1.0F, tex, this.showness, 0.02F, -0.2F, 0.001F, 1.5F, null, null);
   }

   @Override
   public void renderClouds(float partialTicks, WorldClient world, Minecraft mc) {
      if (this.voxelCloudField == null) {
         this.voxelCloudField = new VoxelCloudField(16, 512, 48, 285.0F);
   ...
   public void onPreRender() {
      ...
      this.colors = this.worldProvider.getTimeOfDayProvider().getColors(6, 2, this.worldProvider.getWorld().getWorldTime());
